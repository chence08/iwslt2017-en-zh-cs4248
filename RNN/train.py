...
def run_epoch(data, model, criterion, optimizer=None, is_training=False,clip=1):
    ...
    for batch in tqdm(data):
      i+=1
      output = model(batch.src, batch.src_len,batch.trg)
...
def train(train_data, dev_data, model, criterion, optimizer):
    ...
    for epoch in range(1, config.epoch_num + 1):
        ...
        wandb.log({"valid-loss":dev_loss, "valid-bleu":bleu_score})
...
def evaluate(data, model, mode='dev'):
    ...
    with torch.no_grad():
        for batch in tqdm(data):
            decode_result = model(batch.src, batch.src_len,batch.trg,0)
...
def test(data, model, criterion):
    with torch.no_grad():
        ...
        wandb.log({"test-loss":test_loss, "test-bleu":bleu_score})